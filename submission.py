...
__global__ void __launch_bounds__(WARPGROUP_SIZE + 2 * WARP_SIZE)
  ...
  if (warp_id < 4) { // epilogue warps
    ...
    if constexpr (TRANSPOSE) {
      half *c_off = reinterpret_cast<half *>(C_ptr) +
                    (outer_col * BLOCK_N) * M + (outer_row * BLOCK_M);
      ...
      half *c_off = reinterpret_cast<half *>(C_ptr) +
                    (outer_row * BLOCK_M) * N + (outer_col * BLOCK_N);
      for (int32_t r = 0; r < 2; r++) {
        if constexpr (BLOCK_N == 128) {
    ...
    if (warp_id == 4 && elect_one_sync()) { // issue TMAs
      CachePolicy cache_policy_a = (M == 512 && N == 3072)
                                       ? CachePolicy::EVICT_NORMAL
                                       : CachePolicy::EVICT_LAST;
      CachePolicy cache_policy_b = (M == 512 && N == 3072)
                                       ? CachePolicy::EVICT_NORMAL
                                       : CachePolicy::EVICT_FIRST;
...
template <int32_t K, int32_t BLOCK_M, int32_t BLOCK_N, int32_t BLOCK_K,
          int32_t PIPE_DEPTH, bool FAST_SILU, bool TRANSPOSE>
__global__ void __launch_bounds__(WARPGROUP_SIZE + 2 * WARP_SIZE)
    nvfp4_dual_gemm_swap(int M, int N,
  ...
  constexpr int32_t TILE_SIZE_B = BLOCK_K * BLOCK_M;
  constexpr int32_t TILE_SIZE_A = BLOCK_K * BLOCK_N;
  constexpr int32_t TILE_SIZE_SFB = (BLOCK_K / SF_VEC_SIZE) * 128;
  constexpr int32_t TILE_SIZE_SFA = (BLOCK_K / SF_VEC_SIZE) * 128;
  constexpr int32_t STAGE_BYTES =
      (2 * TILE_SIZE_B + TILE_SIZE_A) / 2 + (2 * TILE_SIZE_SFB + TILE_SIZE_SFA);

  constexpr int32_t TMEM_WIDTH_SFB = 16;
  constexpr int32_t TMEM_WIDTH_SFA = 16;

  const int32_t num_rows = CEIL_DIV(N, BLOCK_M);
  const int32_t num_cols = CEIL_DIV(M, BLOCK_N);
  const int32_t outer_row = blockIdx.x / num_cols;
  const int32_t outer_col = blockIdx.x % num_cols;
  ...
  if (warp_id < 4) { // epilogue warps
    ...
    if constexpr (TRANSPOSE) {
      half *c_off = reinterpret_cast<half *>(C_ptr) +
                    (outer_col * BLOCK_N) * N + (outer_row * BLOCK_M);
      for (int32_t r = 0; r < 2; r++) {
        if constexpr (BLOCK_N == 128) {
          tcgen05_ld_32x32b_x64(tmem_d1 + r * (BLOCK_N / 2), acc1);
          tcgen05_ld_32x32b_x64(tmem_d2 + r * (BLOCK_N / 2), acc2);
        ...
        for (int32_t i = 0; i < (BLOCK_N / 2); i++) {
          float val = silu_func(acc1[i]) * acc2[i];
          __stwt(&c_off[(r * (BLOCK_N / 2) + i) * N + wg_lane_id],
                 __float2half_rn(val));
        }
      }
    } else {
      half *c_off = reinterpret_cast<half *>(C_ptr) +
                    (outer_row * BLOCK_M) * M + (outer_col * BLOCK_N);
      for (int32_t r = 0; r < 2; r++) {
        if constexpr (BLOCK_N == 128) {
          tcgen05_ld_16x256b_x16(tmem_d1 + r * (16 << 16), acc1);
          tcgen05_ld_16x256b_x16(tmem_d2 + r * (16 << 16), acc2);
        ...
        for (int32_t i = 0; i < (BLOCK_N / 2); i += 4) {
          ...
          __stwt(reinterpret_cast<half2 *>(&c_off[row * M + col]),
                 __float22half2_rn(make_float2(val1, val2)));
          __stwt(reinterpret_cast<half2 *>(&c_off[(row + 8) * M + col]),
                 __float22half2_rn(make_float2(val3, val4)));
        }
      }
    }
    ...
    if (warp_id == 4 && elect_one_sync()) { // issue TMAs
      CachePolicy cache_policy_b =
          (M > N) ? CachePolicy::EVICT_LAST : CachePolicy::EVICT_FIRST;
      CachePolicy cache_policy_a =
          (M > N) ? CachePolicy::EVICT_FIRST : CachePolicy::EVICT_LAST;

      auto issue_tma = [&](int32_t k, int32_t pipe_idx) {
        expect_bytes_and_arrive(&tma_done[pipe_idx], STAGE_BYTES);

        char *b1_shr = shmem + pipe_idx * STAGE_BYTES;
        char *b2_shr = b1_shr + TILE_SIZE_B / 2;
        char *a_shr = b2_shr + TILE_SIZE_B / 2;
        char *sfb1_shr = a_shr + TILE_SIZE_A / 2;
        char *sfb2_shr = sfb1_shr + TILE_SIZE_SFB;
        char *sfa_shr = sfb2_shr + TILE_SIZE_SFB;

        cp_async_bulk_tensor_2d_global_to_shared(
            b1_shr, &b1_map, k * BLOCK_K, outer_row * BLOCK_M,
            &tma_done[pipe_idx], cache_policy_b);
        cp_async_bulk_tensor_2d_global_to_shared(
            b2_shr, &b2_map, k * BLOCK_K, outer_row * BLOCK_M,
            &tma_done[pipe_idx], cache_policy_b);
        cp_async_bulk_tensor_2d_global_to_shared(
            a_shr, &a_map, k * BLOCK_K, outer_col * BLOCK_N,
            &tma_done[pipe_idx], cache_policy_a);

        cp_async_bulk_global_to_shared(
            sfb1_shr,
            SFB1_ptr + ((outer_row * BLOCK_M / 128) * ((K / SF_VEC_SIZE) / 4) +
                        k * 4) *
                           512,
            TILE_SIZE_SFB, &tma_done[pipe_idx], cache_policy_b);
        cp_async_bulk_global_to_shared(
            sfb2_shr,
            SFB2_ptr + ((outer_row * BLOCK_M / 128) * ((K / SF_VEC_SIZE) / 4) +
                        k * 4) *
                           512,
            TILE_SIZE_SFB, &tma_done[pipe_idx], cache_policy_b);
        cp_async_bulk_global_to_shared(
            sfa_shr,
            SFA_ptr + ((outer_col * BLOCK_N / 128) * ((K / SF_VEC_SIZE) / 4) +
                       k * 4) *
                          512,
            TILE_SIZE_SFA, &tma_done[pipe_idx], cache_policy_a);
      };
      ...
      const uint32_t tmem_d1 = tmem_base[0];
      const uint32_t tmem_d2 = tmem_d1 + BLOCK_N;
      const uint32_t tmem_sfb1 = tmem_d2 + BLOCK_N;
      const uint32_t tmem_sfb2 = tmem_sfb1 + TMEM_WIDTH_SFB;
      const uint32_t tmem_sfa = tmem_sfb2 + TMEM_WIDTH_SFB;
      ...
      for (int32_t k = 0; k < (K / BLOCK_K); k++) {
        ...
        const uint32_t sfb_offset = (BLOCK_M == 64) ? (outer_row % 2) * 2 : 0;
        const uint32_t sfa_offset = (BLOCK_N == 64) ? (outer_col % 2) * 2 : 0;
        for (int32_t j = 0; j < 4; j++) {
          if (j == 0 && k == 0) {
            tcgen05_mma<0, inst_desc, CollectorUsage::DISCARD>(
                desc_b1 + j * 2, desc_a + j * 2, tmem_d1,
                tmem_sfb1 + j * (TMEM_WIDTH_SFB / 4) + sfb_offset,
                tmem_sfa + j * (TMEM_WIDTH_SFA / 4) + sfa_offset);
            tcgen05_mma<0, inst_desc, CollectorUsage::DISCARD>(
                desc_b2 + j * 2, desc_a + j * 2, tmem_d2,
                tmem_sfb2 + j * (TMEM_WIDTH_SFB / 4) + sfb_offset,
                tmem_sfa + j * (TMEM_WIDTH_SFA / 4) + sfa_offset);
          } else {
            tcgen05_mma<1, inst_desc, CollectorUsage::DISCARD>(
                desc_b1 + j * 2, desc_a + j * 2, tmem_d1,
                tmem_sfb1 + j * (TMEM_WIDTH_SFB / 4) + sfb_offset,
                tmem_sfa + j * (TMEM_WIDTH_SFA / 4) + sfa_offset);
            tcgen05_mma<1, inst_desc, CollectorUsage::DISCARD>(
...
template <int32_t K, int32_t BLOCK_M, int32_t BLOCK_N, int32_t BLOCK_K,
          int32_t PIPE_DEPTH, bool FAST_SILU, bool SWAP, bool TRANSPOSE>
void launch_nvfp4_dual_gemm(int M, int N, uint8_t *A_ptr, uint8_t *B1_ptr,
  ...
  constexpr int32_t A_BLOCK = SWAP ? BLOCK_N : BLOCK_M;
  constexpr int32_t B_BLOCK = SWAP ? BLOCK_M : BLOCK_N;

  CUtensorMap tensorMapA;
  const cuuint64_t globalDimA[2] = {K, M};
  const cuuint64_t globalStridesA[1] = {K / 2};
  const cuuint32_t boxDimA[2] = {BLOCK_K, A_BLOCK};
  ...
  const cuuint32_t boxDimB[2] = {BLOCK_K, B_BLOCK};
  ...
  constexpr int32_t shmem_size_a = PIPE_DEPTH * (BLOCK_K * A_BLOCK) / 2;
  constexpr int32_t shmem_size_b = PIPE_DEPTH * (BLOCK_K * B_BLOCK) / 2;
  ...
  const int32_t num_rows = CEIL_DIV(M, A_BLOCK);
  const int32_t num_cols = CEIL_DIV(N, B_BLOCK);

  if constexpr (SWAP) {
    CUDA_CHECK(cudaFuncSetAttribute(
        nvfp4_dual_gemm_swap<K, BLOCK_M, BLOCK_N, BLOCK_K, PIPE_DEPTH,
                             FAST_SILU, TRANSPOSE>,
        cudaFuncAttributeMaxDynamicSharedMemorySize, shmem_size));
    nvfp4_dual_gemm_swap<K, BLOCK_M, BLOCK_N, BLOCK_K, PIPE_DEPTH, FAST_SILU,
                         TRANSPOSE>
...
#define LAUNCH(K, BLOCK_M, BLOCK_N, BLOCK_K, PIPE_DEPTH, FAST_SILU, SWAP,      \
               TRANSPOSE)                                                      \
  launch_nvfp4_dual_gemm<K, BLOCK_M, BLOCK_N, BLOCK_K, PIPE_DEPTH, FAST_SILU,  \
                         SWAP, TRANSPOSE>(M, N, A_ptr, B1_ptr, B2_ptr,         \
                                          SFA_ptr, SFB1_ptr, SFB2_ptr, C_ptr)
